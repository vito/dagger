...
func (ts *goTypes) schema() []byte {
	...
	for _, s := range ts.Structs {
		...
		*/
		if strings.HasPrefix(s.typ.PkgPath(), "dagger.io/dagger") || strings.HasPrefix(s.typ.PkgPath(), "github.com/dagger/dagger/internal/sdk/dagger") { // TODO(vito): make this ||?
			continue
		}
...
func goReflectTypeToGraphqlType(t reflect.Type, isInput bool) *ast.Type {
	...
	case reflect.String:
		...
		in-progress codegen work.
		*/
		if strings.HasPrefix(t.PkgPath(), "dagger.io/dagger") || strings.HasPrefix(t.PkgPath(), "github.com/dagger/dagger/internal/sdk/dagger") { // TODO(vito): make this ||?
