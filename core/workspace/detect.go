...
type Workspace struct {
	...
	// LegacyModule is the path to a dagger.json that should be auto-loaded
	// as the default module (backwards compat for pre-workspace projects).
	// Set when workspace detection finds a dagger.json without a config.toml.
	LegacyModule string
}
...
func Detect(
	...
	if legacyCloser {
		...
		return &Workspace{Root: legacyDir, LegacyModule: legacyDir}, nil
	}
	...
	if hasLegacy {
		...
		return &Workspace{Root: root, LegacyModule: legacyDir}, nil
	}
