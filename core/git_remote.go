...
func (ref *RemoteGitRef) Tree(ctx context.Context, srv *dagql.Server, discardGitDir bool, depth int, includeTags bool, remotes []GitRemote) (_ *Directory, rerr error) {
	...
	err = MountRef(ctx, checkoutRef, func(checkoutDir string, _ *ctdmount.Mount) error {
		...
		// The checkout owns its objects now. In particular, slow submodules
		// and walking the worktree must not block log/metadata on this URL.
		checkoutRemotes := MergeGitRemotes(
			[]GitRemote{{Name: "origin", URL: ref.repo.URL.Remote()}},
			remotes,
		)
		return finishGitCheckout(ctx, checkoutGit, checkoutRemotes, ref.repo.URL.Remote(), ref.Ref, tmpref, discardGitDir)
	})
	if err != nil {
