...
func (GitSuite) TestGitLog(ctx context.Context, t *testctx.T) {
	...
	t.Run("remote repository", func(ctx context.Context, t *testctx.T) {
		// A bounded log only needs the newest few generations of this repo.
...
func (GitSuite) TestGitLogBoundedRemoteHistory(ctx context.Context, t *testctx.T) {
	...
	// Abbreviated refs resolve only from objects already fetched into the
	// mirror. A cold bounded log must not download this distant ancestor.
	_, err = repo.Ref(root[:12]).CommitSHA(ctx)
	require.Error(t, err)
	requireErrOut(t, err, "already-fetched")
