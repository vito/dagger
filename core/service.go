...
func (svc *Service) runAndSnapshotChanges(
	...
	if running.workspaceSource.Self() == nil || running.workspaceAddress != workspaceAddress {
		...
		for _, entry := range entries {
			if err := root.RemoveAll(entry.Name()); err != nil {
				return res, false, fmt.Errorf("reset service workspace: %w", err)
			}
		}
		running.workspaceSource = dagql.ObjectResult[*Directory]{}
	}
	...
	if running.workspaceSource.Self() != nil {
		changes, err := NewChangeset(ctx, running.workspaceSource, source)
		if err != nil {
			return res, false, err
		}
		...
		for _, path := range paths.AllRemoved {
			if err := root.RemoveAll(strings.TrimSuffix(path, "/")); err != nil {
				return res, false, fmt.Errorf("remove workspace path %q: %w", path, err)
			}
		}
	}
