...
func NewFile(query *Query, def *pb.Definition, file string, pipeline pipeline.Path, platform Platform, services ServiceBindings) *File {
	return &File{
		Query:    query,
		LLB:      def,
		File:     file,
...
func NewFileWithContents(
	...
	dir, err := NewScratchDirectory(query, pipeline, platform).WithNewFile(ctx, name, content, permissions, ownership)
	if err != nil {
		return nil, err
	}
...
func NewFileSt(ctx context.Context, query *Query, st llb.State, dir string, pipeline pipeline.Path, platform Platform, services ServiceBindings) (*File, error) {
	def, err := st.Marshal(ctx, llb.Platform(platform.Spec()))
	if err != nil {
		return nil, err
	}

	return NewFile(query, def.ToPB(), dir, pipeline, platform, services), nil
}
