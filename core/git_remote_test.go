...
func gitMirrorTestRepo(t *testing.T, source string) (*RemoteGitRepository, *gitutil.GitCLI, string) {
	...
	gitBundleTestRun(t, mirror, "remote", "add", "origin", "file://"+source)
	...
	for _, legacy := range []bool{true, false} {
		for _, initialDepth := range []int{0, 1} {
			t.Run(fmt.Sprintf("legacy=%v/depth=%d", legacy, initialDepth), func(t *testing.T) {
				...
				if initialDepth == 1 {
					require.Equal(t, "1", gitBundleTestRun(t, mirror, "rev-list", "--count", base))
				...
				require.Equal(t, "2", gitBundleTestRun(t, mirror, "rev-list", "--count", base))
				...
				require.Equal(t, "3", gitBundleTestRun(t, mirror, "rev-list", "--count", next))
				require.Equal(t, "65536", gitBundleTestRun(t, mirror, "cat-file", "-s", base+"^:history"))
				if !legacy {
...
func TestGitMirrorFetchRetainsExistingCommits(t *testing.T) {
	...
	for _, depth := range []int{0, 1} {
		t.Run(fmt.Sprint(depth), func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Empty(t, gitBundleTestRun(t, mirror, "for-each-ref", "--format=%(refname)"))
			packets, packBytes := gitMirrorTestTrace(t, git, func(git *gitutil.GitCLI) {
				gitMirrorTestFetch(t, repo, git, base, 0)
			})
			require.Equal(t, base, gitBundleTestRun(t, mirror, "rev-parse", fetchedGitRef(base)))
			if depth == 0 {
...
func TestGitMirrorFetchPrivateRefs(t *testing.T) {
	...
	require.Equal(t, "1", gitBundleTestRun(t, mirror, "rev-list", "--count", next), "successive shallow fetches must stay shallow")
	require.Empty(t, gitBundleTestRun(t, mirror, "for-each-ref", "--format=%(refname)", "refs/heads", "refs/tags", "refs/remotes"))
	require.Equal(t, base, gitBundleTestRun(t, mirror, "rev-parse", fetchedGitRef(base)))
	require.Equal(t, next, gitBundleTestRun(t, mirror, "rev-parse", fetchedGitRef(next)))
	checkout := t.TempDir()
	...
	require.Empty(t, gitBundleTestRun(t, checkout, "for-each-ref", "--format=%(refname)"), "private mirror refs and unrequested tags must not leak into checkout")
	require.Equal(t, "new", gitBundleTestRun(t, checkout, "show", "HEAD:small"))
}
	...
	for _, depth := range []int{0, 1} {
		t.Run(fmt.Sprint(depth), func(t *testing.T) {
			...
			if depth == 0 {
				require.NoError(t, err)
				require.Equal(t, base, gitBundleTestRun(t, mirror, "rev-parse", fetchedGitRef(base)))
				require.Equal(t, fetchedGitRef(base), gitBundleTestRun(t, mirror, "for-each-ref", "--format=%(refname)", "refs/dagger.fetched"))
				require.Empty(t, gitBundleTestRun(t, mirror, "for-each-ref", "--format=%(refname)", "refs/dagger.fetch", "refs/heads", "refs/tags"))
			} else {
				require.ErrorContains(t, err, "did not materialize expected sha")
				require.Empty(t, gitBundleTestRun(t, mirror, "for-each-ref", "--format=%(refname)", "refs/dagger.fetched", "refs/dagger.fetch"), "failed fallback must not retain an incorrect tip")
			}
		})
	}
}
