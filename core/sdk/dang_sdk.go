...
func (r *DangRuntime) Call(
	...
	// dag, err := fnCall.Module.Deps.Schema(ctx)
	// if err != nil {
	// 	return nil, "", fmt.Errorf("get schema dag: %w", err)
	// }

	// q := dag.Root().Unwrap().(*core.Query)
	q := r.root
	...
	gql := graphql.NewClient(fmt.Sprintf("http://%s/query", l.Addr()), nil)
			...
			for _, slot := range val.Parameters {
				for _, dir := range slot.Directives {
					...
					for _, arg := range dir.Args {
						...
						if err != nil {
							return res, fmt.Errorf("failed to evaluate directive argument %s.%s.%s: %w", slot.Named, dir.Name, arg.Key, err)
						}
...
func createFunction(ctx context.Context, srv *dagql.Server, name string, fn *hm.FunctionType, directives ProcessedDirectives) (dagql.ObjectResult[*core.Function], error) {
	...
	for _, arg := range fn.Arg().(*dang.RecordType).Fields {
		...
		// 	if err != nil {
		// 		return res, fmt.Errorf("failed to marshal default value for %s: %w", arg.Key, err)
		// 	}
		// 	argArgs = append(argArgs, dagql.NamedInput{Name: "defaultValue", Value: core.JSON(js)})
		// }
...
func dangTypeToTypeDef(ctx context.Context, srv *dagql.Server, dangType hm.Type) (dagql.ObjectResult[*core.TypeDef], error) {
	...
	if nonNull, isNonNull := dangType.(hm.NonNullType); isNonNull {
		...
		// Chain withOptional onto the inner result by starting from srv.Root() and selecting the inner typedef
