...
func (s *Query) CurrentServedDeps(ctx context.Context) (*ModDeps, error) {
	clientMetadata, err := engine.ClientMetadataFromContext(ctx)
	if err != nil {
		return nil, err
	}
...
func (query *Query) WithPipeline(name, desc string, labels []pipeline.Label) *Query {
	query = query.Clone()
	query.Pipeline = query.Pipeline.Add(pipeline.Pipeline{
		Name:        name,
		Description: desc,
		Labels:      labels,
	})
