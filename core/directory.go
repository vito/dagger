...
func NewDirectory(query *Query, def *pb.Definition, dir string, pipeline pipeline.Path, platform Platform, services ServiceBindings) *Directory {
	if query == nil {
...
func NewScratchDirectory(query *Query, pipeline pipeline.Path, platform Platform) *Directory {
	if query == nil {
		panic("query must be non-nil")
	}
	return &Directory{
...
func NewDirectorySt(ctx context.Context, query *Query, st llb.State, dir string, pipeline pipeline.Path, platform Platform, services ServiceBindings) (*Directory, error) {
	if query == nil {
		panic("query must be non-nil")
	}
	...
	return NewDirectory(query, def.ToPB(), dir, pipeline, platform, services), nil
}
...
func (dir *Directory) WithNewFile(ctx context.Context, dest string, content []byte, permissions fs.FileMode, ownership *Ownership) (*Directory, error) {
	...
	err = dir.SetState(ctx, st) // XXX(vito) does this mutate properly?
	if err != nil {
		return nil, err
	}
